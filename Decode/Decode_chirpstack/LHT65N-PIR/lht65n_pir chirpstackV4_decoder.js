...
if(Ext==0x09)
{
  decode.temperature_pro=parseFloat(((bytes[0]<<24>>16 | bytes[1])/100).toFixed(2));
...
if(Ext!=0x0f)
{
  ...
  decode.humidity=parseFloat((((bytes[4]<<8 | bytes[5])&0xFFF)/10).toFixed(1));
}
if(Connect=='1')
{
...
else if(Ext=='1')
{
  ...
  decode.temperature_pro_ds=parseFloat(((bytes[7]<<24>>16 | bytes[8])/100).toFixed(2));
}
else if(Ext=='2')
{
  decode.Ext_sensor ="Temperature Sensor";
  decode.temperature_pro_117=parseFloat(((bytes[7]<<24>>16 | bytes[8])/100).toFixed(2));
}
else if(Ext=='4')
{
